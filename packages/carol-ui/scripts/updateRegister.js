...
const componentsTsPath = path.resolve(__dirname, '../packages/components/components.ts')
...
const customThemeTsPath = path.resolve(
  __dirname,
  '../packages/components/config-provider/src/custom-theme.ts'
)
...
const filterComponentNames = componentNames.filter(({ name }) => {
  ...
  const themePath = path.resolve(
    __dirname,
    `../packages/components/${kebabCase(name)}/styles/index.ts`
  )
...
filterComponentNames.forEach(({ name }) => {
  ...
  writeFileByLine(
    customThemeTsPath,
    `import type { ${themeName} } from '@/components/${kebabCaseName}/styles'`
  )
})
...
const themeLightTsPath = path.resolve(__dirname, '../packages/themes/light.ts')
...
filterComponentNames.forEach(({ name }) => {
  ...
  writeFileByLine(
    themeLightTsPath,
    `import { ${lightName} } from '@/components/${kebabCaseName}/styles'`
  )
})
writeFileByLine(themeLightTsPath)
